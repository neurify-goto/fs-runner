...
from tests.data.test_client_data import CLIENT_DATA, TARGETING_DATA, create_test_client_config
...
class FieldMappingAnalyzer:
    ...
    async def fetch_test_form_url(self, company_id: Optional[int] = None) -> Optional[Dict[str, Any]]:
        ...
        try:
            if company_id:
                logger.info(f"Fetching specific company (ID: {company_id}) from database...", extra={'summary': True})
                
                # 指定されたIDの企業を取得
                # フォームURLは http(s) のみを対象にする（mailto等でのブラウザ終了回避）
                response = self.supabase_client.table('companies')\
                ...
                if not response.data:
                    logger.error(f"Company with ID {company_id} not found or has no form_url")
                    return None
            ...
            logger.info(f"   Has instruction: {'Yes' if company.get('instruction_json') else 'No'}")
    ...
    async def _analyze_form_mapping_once(self, form_url: str) -> Tuple[Dict[str, Any], str]:
        """フォームマッピング解析実行（単回実行）"""
        ...
        try:
            ...
            if form_count == 0 or has_hubspot_script:
                if form_count == 0:
                    logger.info("No form elements found with domcontentloaded, trying additional strategies...")
                else:
                ...
                if success:
                    form_count = await self.page.evaluate("document.querySelectorAll('form').length")
                    logger.info(f"📋 Form elements found after dynamic waiting: {form_count}")
                    ...
                    if has_hubspot_script:
                        ...
                        """)
                        logger.info(f"📋 HubSpot elements: containers={hubspot_info['hbsptForms']}, inputs={hubspot_info['hsInputs']}, fieldsets={hubspot_info['hsFieldsets']}")
            ...
            analysis_result = await analyzer.analyze_form(client_data=create_test_client_config())
        ...
        for attempt in range(2):
            try:
                ...
                if 'has been closed' in str(e):
                    logger.info("Detected unexpected page/context/browser close. Recreating page and retrying once...")
                    await self._recreate_page()
                    continue
                raise
        # 再試行しても失敗した場合
    ...
    def analyze_mapping_results(self, analysis_result: Dict[str, Any]) -> Dict[str, Any]:
        """マッピング結果の詳細分析"""
        logger.info("\n" + "="*80)
        ...
        for field_name, field_info in field_mappings.items():
            ...
            # デバッグログ追加
            logger.debug(f"Element type: {element_type_name}, Element str: {element_str[:100]}...")
            ...
            if 'Locator' in element_type_name or 'Locator' in element_str:
                ...
                element_name = element_str
                element_id = 'Unknown'
                element_type = element_type_name.split("'")[1] if "'" in element_type_name else 'Unknown'
                selector = element_str
        ...
        if 'form_sender_name' in field_mappings or any('form_sender_name' in str(info) for info in field_mappings.values()):
        ...
        if duplicates:
            ...
            non_email_duplicates = {v: count for v, count in duplicates.items() 
            ...
            if non_email_duplicates:
                ...
                logger.warning(f"⚠️  Non-email duplicate values found (count={len(non_email_duplicates)})", extra={'summary': True})
        ...
        if required_info and not required_info.get('error'):
            ...
            for req_element in required_info.get('required_elements', []):
                label = req_element.get('label_text', req_element.get('placeholder', req_element.get('name', 'N/A')))
        ...
        logger.info("   ℹ️  Detailed evaluation will be performed by field-mapping-evaluator agent")
    ...
    def _check_field_issues(self, field_name: str, field_info: Dict[str, Any]) -> List[str]:
        """個別フィールドの問題チェック"""
        issues = []
        value = field_info.get('value', '')
        score = field_info.get('score', 0)
        ...
        # 不適切な値の例（五十嵐問題等）
        if field_name in ['会社名', 'company_name'] and any(name in value for name in ['五十嵐', '駿', 'いがらし', 'しゅん']):
            issues.append('name_in_company_field')
        
        if field_name in ['姓', '名', 'last_name', 'first_name'] and '株式会社' in value:
    ...
    async def run_single_test(self, company_id: Optional[int] = None) -> bool:
        ...
        try:
            ...
            # 結果をJSONファイルに保存
            timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
            result_file = os.path.join(self.temp_dir, f"analysis_result_{timestamp}.json")
    ...
    def _extract_required_fields(self, form_content: str) -> Dict[str, Any]:
        ...
        try:
            if not HAS_BEAUTIFULSOUP:
                return {'error': 'BeautifulSoup not available for required field detection'}
    ...
    async def _wait_for_dynamic_content(self, max_wait: int = 15) -> bool:
        ...
        try:
            ...
            for selector in hubspot_selectors:
                try:
                    ...
                    if count > 0:
                        logger.info(f"✅ HubSpot form detected with selector '{selector}': {count} forms")
                        return True
                except Exception:
                    continue
        ...
        try:
            ...
            form_count = await self.page.evaluate("document.querySelectorAll('form').length")
            if form_count > 0:
                logger.info(f"✅ Form elements found after extended networkidle: {form_count}")
                return True
            ...
            total_hubspot = hubspot_elements['hbsptForms'] + hubspot_elements['hsInputs'] + hubspot_elements['hsFieldsets']
            if total_hubspot > 0:
                logger.info(f"🔍 HubSpot elements found: containers={hubspot_elements['hbsptForms']}, inputs={hubspot_elements['hsInputs']}, fieldsets={hubspot_elements['hsFieldsets']}")
                # HubSpotコンテナがあっても、実際のinput要素がない場合は継続して待機
                if hubspot_elements['hsInputs'] > 0 or hubspot_elements['hsFieldsets'] > 0:
                    logger.info("✅ HubSpot input elements detected, form is ready")
                    return True
                else:
                    logger.info("HubSpot containers found but no input elements yet - continuing to Strategy 3...")
            ...
            if input_count > 2:  # 最低3つのinput要素が必要
                logger.info(f"✅ Multiple input elements found without form tag: {input_count}")
                return True
        ...
        try:
            ...
            if has_hubspot:
                logger.info("HubSpot detected - applying enhanced form generation wait...")
                ...
                for attempt in range(4):  # 最大4回試行（5秒x4 = 20秒）
                    ...
                    try:
                        ...
                        for frame in frames:
                            if frame != self.page.main_frame:  # メインフレーム以外をチェック
                                frame_forms = await frame.evaluate("document.querySelectorAll('form').length")
                                frame_inputs = await frame.evaluate("document.querySelectorAll('input').length")
                                iframe_form_count += frame_forms
                                iframe_input_count += frame_inputs
                                if frame_forms > 0:
                                    logger.info(f"iframe detected forms: {frame_forms} in frame: {frame.url}")
                    except Exception as e:
                    ...
                    logger.info(f"HubSpot attempt {attempt+1}: main_forms={form_status['forms']}, iframe_forms={iframe_form_count}, hsInputs={form_status['hsInputs']}, hsFieldsets={form_status['hsFieldsets']}, total_inputs={total_inputs}")
                    
                    # 成功条件：実際のform要素またはinput要素が十分数存在
                    if (total_forms > 0 or 
                        form_status['hsInputs'] > 3 or 
                        form_status['hsFieldsets'] > 0 or
                        total_inputs > 5):
                        logger.info(f"✅ HubSpot form elements fully loaded on attempt {attempt+1} (forms: {total_forms}, inputs: {total_inputs})")
                        return True
                
                # HubSpot用のiframe処理
                iframe_count = await self.page.evaluate("document.querySelectorAll('iframe.hs-form-iframe').length")
                if iframe_count > 0:
                    logger.info(f"HubSpot iframe detected: {iframe_count}, applying additional wait...")
            ...
            total_elements = final_check['forms'] + final_check['inputs'] + final_check['textareas'] + final_check['selects']
            logger.info(f"JavaScript wait results: forms={final_check['forms']}, hbspt={final_check['hbsptForms']}, inputs={final_check['inputs']}, hsInputs={final_check['hsInputs']}, textareas={final_check['textareas']}, selects={final_check['selects']}")
            
            if total_elements > 0 or final_check['hsInputs'] > 0:
                logger.info(f"✅ Form elements found via JavaScript wait: {total_elements} total (hsInputs: {final_check['hsInputs']})")
                return True
        ...
        try:
            ...
            if form_count > 0 or input_count > 0:
                logger.info(f"✅ Elements found after scroll: forms={form_count}, inputs={input_count}")
                return True
                
        except Exception as e:
            logger.debug(f"Scroll trigger failed: {e}")
        
        logger.warning("⚠️ No form elements found after all dynamic strategies (including HubSpot)")
        return False
    ...
    async def _analyze_iframes(self) -> Tuple[str, Optional[Any]]:
                    ...
                    try:
                        ...
                        if not HAS_BEAUTIFULSOUP:
                            ...
                            if '<form' in frame_content.lower():
                                iframe_count += 1
                                iframe_contents.append(f"<!-- iframe {iframe_count} Content from {frame.url} -->\n{frame_content}\n")
                            ...
                            if forms:
                                iframe_count += 1
                                iframe_contents.append(f"<!-- iframe {iframe_count} Forms from {frame.url} -->\n")
                                for i, form in enumerate(forms):
                                    iframe_contents.append(f"<!-- iframe {iframe_count} Form {i+1} -->\n{str(form)}\n")
                                
                                logger.info(f"Extracted {len(forms)} form(s) from iframe: {frame.url}")
                            ...
                            hubspot_elements = soup.find_all(['input', 'textarea', 'fieldset'], 
                                                           class_=lambda x: x and ('hs-' in str(x) if x else False))
                            if hubspot_elements and not forms:
                                # フォームタグがないがHubSpot要素がある場合
                                iframe_count += 1
                                body = soup.find('body') or soup
                                iframe_contents.append(f"<!-- iframe {iframe_count} HubSpot Elements from {frame.url} -->\n{str(body)}\n")
                                logger.info(f"Extracted HubSpot elements from iframe: {frame.url}")
        ...
        if target_frame:
            logger.info(f"📋 Target iframe found for analysis: {len(await target_frame.query_selector_all('form'))} forms found")
    ...
    def _extract_form_content(self, page_html: str) -> str:
        ...
        try:
            ...
            hubspot_containers = soup.find_all(['div'], class_=['hbspt-form', 'hs-form'])
            hubspot_forms_by_id = soup.find_all('div', id=lambda x: x and x.startswith('hbspt-form-'))
            ...
            if form_elements:
                ...
                logger.info(f"Extracted {len(form_elements)} standard form element(s) from page source")
            
            # HubSpotフォームコンテナを抽出
            hubspot_count = 0
            for container in hubspot_containers + hubspot_forms_by_id:
                if container not in [elem.find_parent(['div'], class_=['hbspt-form', 'hs-form']) for elem in form_elements if elem.find_parent(['div'], class_=['hbspt-form', 'hs-form'])]:
                    hubspot_count += 1
                    form_contents.append(f"<!-- HubSpot Container {hubspot_count} -->\n{str(container)}\n")
            
            if hubspot_count > 0:
                logger.info(f"Extracted {hubspot_count} HubSpot form container(s) from page source")
            ...
            if not form_contents:
                ...
                if len(inputs) > 3:
                    logger.warning(f"No form containers found, but {len(inputs)} input elements detected - extracting body")
                    body = soup.find('body')
                    if body:
    ...
    def _extract_form_basic(self, page_html: str) -> str:
        ...
        return '\n\n'.join(f"<!-- Form {i+1} (basic extraction) -->\n{form}" 
                          for i, form in enumerate(forms))
...
async def run_continuous_tests(num_tests: int = 3, company_id: Optional[int] = None):
    """継続的テスト実行 - リソース管理強化版"""
    if company_id:
        logger.info(f"🔄 Starting continuous field mapping analysis with company ID {company_id} ({num_tests} tests)")
    else:
        logger.info(f"🔄 Starting continuous field mapping analysis ({num_tests} tests)")
    ...
    for i in range(num_tests):
        ...
        # コンテキストマネージャーで確実なリソース管理
        try:
            async with FieldMappingAnalyzer() as analyzer:
                success = await analyzer.run_single_test(company_id)
            ...
            # リソース解放を確実にするため、追加のガベージコレクション
            import gc
            gc.collect()
        
        # テスト間隔とメモリクリーンアップ
        if i < num_tests - 1:
            logger.info(f"Waiting 3 seconds before next test...")
...
async def main():
    ...
    parser = argparse.ArgumentParser(
        description='Field Mapping Algorithm Testing Tool',
...
Examples:
  ...
  python test_field_mapping_analyzer.py --count 3          # Run 3 random tests
  python test_field_mapping_analyzer.py --count 3 --company-id 12345  # Run 3 tests with same company
"""
    )
    
    parser.add_argument('--company-id', type=int, help='Specific company ID to test')
    parser.add_argument('--count', type=int, help='Number of tests to run (for continuous testing)')
    ...
    # 連続テストか単一テストかを判定
    if args.count:
        await run_continuous_tests(args.count, args.company_id)
    else:
        # 単一テスト実行 - コンテキストマネージャーで確実なリソース管理
        try:
            async with FieldMappingAnalyzer() as analyzer:
                success = await analyzer.run_single_test(args.company_id)
