...
class SelfClosingPageError(Exception):
    """対象ページが読み込み直後に自己終了する（自動化耐性/スキーム遷移等）ケースを表す内部例外。"""
    pass
...
class FieldMappingAnalyzer:
    ...
    async def _initialize_browser(self):
        ...
        try:
            ...
            # Context 生成 → 共通設定 → Page 生成（順序を明確化）
            self.context = await self.browser.new_context()
            await self._configure_context()
            self.page = await self.context.new_page()
            try:
                self.page.on(
                    "close", lambda: logger.debug("[hook] page close event captured")
                )
            except Exception:
                pass
    ...
    async def _configure_context(self) -> None:
        """Context に共通のルーティング/ヘッダ/初期スクリプトを適用する。"""
        assert self.context is not None

        # 不要リソースのブロッキング（速度最適化）。'other' はブロックしない（安定性優先）。
        async def handle_route(route):
            resource_type = route.request.resource_type
            url = route.request.url.lower()

            if resource_type in ["image", "media", "font", "manifest"]:
                await route.abort()
                return
        ...
        # 予期せぬ window.close() によるページ終了を無効化
        await self.context.add_init_script("""
            (() => {
              try {
                // 強制的に close を無害化（prototype 経由も含む）
                const blocker = function(){ try { console.debug('window.close blocked'); } catch(e) {} };
                try { window.close = blocker; } catch(e) {}
                try { self.close = blocker; } catch(e) {}
                try { top.close = blocker; } catch(e) {}
                try {
                  const desc = Object.getOwnPropertyDescriptor(Window.prototype, 'close');
                  if (desc && desc.configurable) {
                    Object.defineProperty(Window.prototype, 'close', { value: blocker, configurable: true, writable: false });
                  }
                } catch(e) {}
              } catch(e) {}

              try { window.open = function(url, target, features){ try { window.location.href = url || ''; } catch(e) {} return null; }; } catch(e) {}
              try { window.onbeforeunload = null; } catch(e) {}
            })();
        """)

        # 生成されるすべてのページにイベントフック（診断用）
        try:
            self.context.on("page", lambda p: p.on("close", lambda: logger.debug("[hook] page close event captured(context)")))
        except Exception:
            pass
                ...
                if self.context is None:
                    ...
                    try:
                        self.page.on(
                            "close",
                            lambda: logger.debug(
                                "[hook] page close event captured(recreated)"
                            ),
                        )
                    except Exception:
                        pass
    ...
    async def _analyze_form_mapping_once(
        ...
        """フォームマッピング解析実行（単回実行）"""
        logger.info("Starting form mapping analysis...", extra={"summary": True})
    ...
    async def analyze_form_mapping(self, form_url: str) -> Tuple[Dict[str, Any], str]:
        ...
        for attempt in range(3):
            try:
                ...
                raise
        # 再試行しても失敗した場合
        if page_closed_count >= 2:
            # 自己終了タイプと分類して上位でスキップ扱い
            raise SelfClosingPageError("page self-closed repeatedly")
    ...
    async def run_single_test(self, company_id: Optional[int] = None) -> bool:
        ...
        try:
            ...
            except SelfClosingPageError:
                # 自己終了ページはスキップし、別のテスト対象を自動再取得（再発防止）
                logger.warning(
                    "Self-closing page detected. Selecting another test company to avoid flakiness...",
                    extra={"summary": True},
                )
                alt = await self.fetch_test_form_url(None)
                if not alt:
                    logger.error("No alternative test company available")
                    return False
                form_url = alt["form_url"]
                analysis_result, source_file = await self.analyze_form_mapping(form_url)
...
async def run_continuous_tests(num_tests: int = 3, company_id: Optional[int] = None):
    ...
    if company_id:
        logger.info(
            f"🔄 Starting continuous field mapping analysis with company ID {company_id} ({num_tests} tests)"
        )
    else:
        logger.info(
            f"🔄 Starting continuous field mapping analysis ({num_tests} tests)"
        )
...
""",
    ...
    parser.add_argument(
        "--count", type=int, help="Number of tests to run (for continuous testing)"
    )
    parser.add_argument(
