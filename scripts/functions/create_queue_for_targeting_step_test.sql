...
create or replace function public.create_queue_for_targeting_step_test(
  p_target_date date,
...
begin
  ...
  if v_stage = 1 then
    ...
    v_sql := v_sql || ' order by c.id asc limit $5 ),
      ...
      ins as (
        insert into public.send_queue_test(target_date_jst, targeting_id, company_id, priority, shard_id, status, attempts, created_at)
    ...
    -- Stage2: 成功履歴なし（当日未送信）かつ直近2週間(JST)に送信試行が無い
    v_sql :=
      'with hist as (
...
grant execute on function public.create_queue_for_targeting_step_test(date, bigint, text, text, integer, integer, bigint, smallint, integer)
