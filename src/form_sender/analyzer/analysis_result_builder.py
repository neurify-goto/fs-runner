...
class AnalysisResultBuilder:
    ...
    def create_analysis_summary(self, field_mapping: Dict[str, Any], 
                                ...
                                special_elements: Dict[str, List[Any]]) -> Dict[str, Any]:
        ...
        return {
            ...
            'important_fields_mapped': f"{mapped_important}/{len(important_fields)}",
            'special_elements_count': {k: len(v) for k, v in special_elements.items()},
            'analysis_success': mapped_count > 0 or auto_handled_count > 0
        }
