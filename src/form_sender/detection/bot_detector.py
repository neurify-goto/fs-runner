...
class BotDetectionSystem:
    ...
    async def detect_bot_protection(page: Page) -> Tuple[bool, Optional[str]]:
        """Bot保護システムを検出（超厳格版 - 偽陽性防止最優先）"""
        try:
            # Step 1: 除外条件チェック（通常ページの特徴があればbot protectionではない）
            if await BotDetectionSystem._is_normal_page(page):
                return False, None
            
            # Step 2: 確実なreCAPTCHA検出（非常に厳格）
            ...
            # Step 3: 確実なCloudflare Challenge検出（非常に厳格）
    ...
    async def _detect_strict_recaptcha(page: Page) -> Tuple[bool, Optional[str]]:
        """厳格なreCAPTCHA検出（複数条件をANDで組み合わせ）"""
        try:
            # 条件1: 実際のreCAPTCHA iframeが存在かつ表示中
            recaptcha_iframe = await page.locator('iframe[src*="recaptcha/api2/anchor"]').count()
            if recaptcha_iframe == 0:
                return False, None
            
            # 条件2: .g-recaptcha要素がdata-sitekey属性付きで存在
            g_recaptcha = await page.locator('.g-recaptcha[data-sitekey]').count()
            if g_recaptcha == 0:
                return False, None
            
            # 条件3: reCAPTCHA要素が実際に表示されている
            visible_recaptcha = await page.locator('.g-recaptcha').is_visible()
            if not visible_recaptcha:
                return False, None
            
            # 条件4: 通常のページコンテンツが少ない（reCAPTCHAページの特徴）
            text_content = await page.locator('body').inner_text()
            if len(text_content.strip()) > BotDetectionThresholds.RECAPTCHA_PAGE_MAX_TEXT_LENGTH:
                return False, None
            
            # 全条件を満たした場合のみreCAPTCHA検出
