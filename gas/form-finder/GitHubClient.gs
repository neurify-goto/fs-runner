/**
 * GitHub API統合モジュール（Form Finder用）
...
function getEventTypeFromTaskType(taskType) {
  const eventTypeMapping = {
    'form_finder': 'form_finder_task'
  };
  
  return eventTypeMapping[taskType] || 'form_finder_task';
}
...
/**
 * ワークフロー判定の堅牢化機能（Form Finder専用）
 */

/**
 * ワークフローがform_finder_taskかどうかを複数条件で判定
 * @param {Object} workflowRun GitHub Actions workflow run オブジェクト
 * @returns {boolean} form_finder_taskワークフローかどうか
 */
function isFormFinderWorkflow(workflowRun) {
  const criteria = {
    nameMatch: false,
    pathMatch: false,
    eventMatch: false,
  ...
  try {
    // 1. ワークフロー名による判定
    if (workflowRun.name) {
      criteria.nameMatch = workflowRun.name === 'Form Finder' ||
                          workflowRun.name.toLowerCase().includes('form-finder') ||
                          workflowRun.name.toLowerCase().includes('form_finder');
    }
    
    // 2. ワークフローファイルパスによる判定
    if (workflowRun.path) {
      criteria.pathMatch = workflowRun.path.includes('form-finder.yml') ||
                          workflowRun.path.includes('form-finder.yaml') ||
                          workflowRun.path.includes('form_finder.yml') ||
                          workflowRun.path.includes('form_finder.yaml');
    }
    ...
    if (workflowRun.head_branch) {
      ...
      criteria.headBranchMatch = ['main', 'master', 'develop'].includes(workflowRun.head_branch) ||
                                workflowRun.head_branch.includes('feature/form-finder');
    }
    ...
    return workflowRun.name === 'Form Finder';
  }
}
 ...
 */
function checkRunningFormFinderWorkflows() {
  const functionKey = 'checkRunningFormFinderWorkflows';
  const config = CONFIG;
  ...
  try {
    ...
    if (responseCode === 200) {
      ...
      // form_finder_taskワークフローのみフィルタ（堅牢化版）
      const formFinderRuns = data.workflow_runs.filter(run => {
        return isFormFinderWorkflow(run);
      });
      
      const runningCount = formFinderRuns.length;
      
      console.log(`form_finder_taskワークフロー並列実行数: ${runningCount}個`);
      
      if (runningCount > 0) {
        console.log('実行中のform_finder_taskワークフロー:');
        formFinderRuns.forEach((run, index) => {
      ...
      return {
        ...
        runningWorkflows: formFinderRuns.map(run => ({
          id: run.id,
          created_at: run.created_at,
          status: run.status
        }))
...
function testWorkflowIdentification(testWorkflowRun) {
  console.log('=== Form Finder ワークフロー判定テスト ===');
  ...
  const sampleWorkflows = testWorkflowRun ? [testWorkflowRun] : [
    ...
    {
      id: 3,
      name: 'Form Finder Legacy',
      path: '.github/workflows/form_finder_old.yml',
      event: 'repository_dispatch',
      head_branch: 'feature/form-finder-update'
    },
    {
      id: 4,
      name: 'Other Workflow',
